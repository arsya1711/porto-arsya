-- Pisahkan periode ketersediaan ujian dari durasi pengerjaan siswa.
-- starts_at/ends_at mengatur kapan ujian tersedia, sedangkan duration_minutes
-- membatasi waktu siswa sejak attempt dimulai. Deadline attempt tetap dibatasi
-- oleh ends_at melalui public.exam_attempt_deadline.

drop function if exists public.save_managed_exam(
  uuid, text, text, uuid, uuid, timestamptz, integer, public.exam_status,
  uuid[], text, boolean, boolean, boolean, boolean
);

create function public.save_managed_exam(
  target_exam_id uuid,
  exam_title text,
...
begin
  ...
  if end_time is null or end_time <= start_time then
  ...
  if duration_in_minutes is null or duration_in_minutes < 1 then
    raise exception 'Batas waktu pengerjaan minimal 1 menit';
  end if;
...
revoke all on function public.save_managed_exam(
  uuid, text, text, uuid, uuid, timestamptz, timestamptz, integer,
  public.exam_status, uuid[], text, boolean, boolean, boolean, boolean
) from public, anon;
...
comment on function public.save_managed_exam(
  uuid, text, text, uuid, uuid, timestamptz, timestamptz, integer,
  public.exam_status, uuid[], text, boolean, boolean, boolean, boolean
) is 'Menyimpan periode ketersediaan ujian dan durasi pengerjaan siswa secara terpisah.';
